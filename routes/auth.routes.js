...
const checkLoogedInUser = (req, res, next) => {
  (req.session.loggedInUser) ? next() : res.redirect('/signin')
}

router.get('/profile', checkLoogedInUser, (req, res) => {
    let email = req.session.loggedInUser.email
    res.render('profile.hbs', {email})
})

router.get('/logout', (req,res) => {
  //get rid if the session
  req.session.destroy()
  res.redirect('/')
})
